...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, '../output/translated-reviews.json');
const UPDATE_PROGRESS_FILE = path.join(__dirname, '../output/update-progress.json');
...
function mergeLocalizedText(
  ...
  newTranslations: { en_US: string; th_TH: string },
): LocalizedText {
 ...
 * 메인 업데이트 함수
 */
async function updateReviewsTranslation() {
  try {
    console.log('🔄 리뷰 번역 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedReview) => {
        try {
          ...
          // 기존 데이터와 새로운 번역 병합
          const updatedTitle = mergeLocalizedText(currentReview.title, {
            en_US: translatedReview.title.en_US,
            th_TH: translatedReview.title.th_TH,
          });

          const updatedContent = mergeLocalizedText(currentReview.content, {
            en_US: translatedReview.content.en_US,
            th_TH: translatedReview.content.th_TH,
          });
...
export { updateReviewsTranslation };
