...
async function translateLiveReviewsToJaJP() {
  try {
    console.log('🌐 실시간 리뷰(LiveReview) 일본어(ja-JP) 번역 작업 시작...');
    ...
    // 번역이 필요한 실시간 리뷰만 필터링 (ja_JP가 없거나 비어있는 경우)
    ...
    console.log(
      `📊 총 ${allLiveReviews.length}개 실시간 리뷰 중 ${totalCount}개 실시간 리뷰가 일본어 번역이 필요합니다.`,
    );
