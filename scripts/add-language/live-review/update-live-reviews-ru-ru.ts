...
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-live-reviews-ru-ru.json');
...
async function updateLiveReviewsRuRU() {
  try {
    console.log('🔄 실시간 리뷰(LiveReview) 러시아어(ru-RU) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedLiveReview) => {
        try {
          ...
          const updateData: { content?: LocalizedText } = {};
          if (translatedLiveReview.content.ru_RU) {
            updateData.content = mergeLocalizedTextWithRuRU(
              currentLiveReview.content,
              translatedLiveReview.content.ru_RU,
            );
          }
