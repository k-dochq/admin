...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-live-reviews-tl-ph.json');
 ...
 * 메인 업데이트 함수
 */
async function updateLiveReviewsTlPH() {
  try {
    console.log('🔄 실시간 리뷰(LiveReview) 필리핀어(tl-PH) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedLiveReview) => {
        try {
          ...
          if (translatedLiveReview.content.tl_PH) {
            updateData.content = mergeLocalizedTextWithTlPH(
              currentLiveReview.content,
              translatedLiveReview.content.tl_PH,
            );
          }
