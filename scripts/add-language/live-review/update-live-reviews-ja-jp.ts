...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-live-reviews-ja-jp.json');
 ...
 * 메인 업데이트 함수
 */
async function updateLiveReviewsJaJP() {
  try {
    console.log('🔄 실시간 리뷰(LiveReview) 일본어(ja-JP) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedLiveReview) => {
        try {
          ...
          if (translatedLiveReview.content.ja_JP) {
            updateData.content = mergeLocalizedTextWithJaJP(
              currentLiveReview.content,
              translatedLiveReview.content.ja_JP,
            );
          }
