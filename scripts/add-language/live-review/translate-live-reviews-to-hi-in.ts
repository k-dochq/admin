...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-live-reviews-hi-in.json');
 ...
 * 메인 번역 함수
 */
async function translateLiveReviewsToHiIN() {
  try {
    console.log('🌐 실시간 리뷰(LiveReview) 힌디어(hi-IN) 번역 작업 시작...');
    ...
    // 번역이 필요한 실시간 리뷰만 필터링 (hi_IN이 없거나 비어있는 경우)
    const liveReviewsToTranslate = allLiveReviews.filter((liveReview) => {
      ...
      // content 번역이 필요하면 포함
      return needsHiINTranslation(liveReview.content);
    });

    const totalCount = liveReviewsToTranslate.length;
    console.log(
      `📊 총 ${allLiveReviews.length}개 실시간 리뷰 중 ${totalCount}개 실시간 리뷰가 힌디어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const liveReview of liveReviews) {
        // content 번역
        if (needsHiINTranslation(liveReview.content)) {
      ...
      for (const liveReview of liveReviews) {
        ...
        const translatedLiveReview: TranslatedLiveReview = {
          ...
          content: {
            ...
            ja_JP: contentJaJP,
            hi_IN:
              translations.content_hi_IN ||
              getTextByLocale(liveReview.content, 'hi_IN') ||
              contentKo,
          },
        };
...
export { translateLiveReviewsToHiIN };
