...
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-live-reviews-ar-sa.json');
...
async function updateLiveReviewsArSA() {
  try {
    console.log('🔄 실시간 리뷰(LiveReview) 아랍어(ar-SA) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedLiveReview) => {
        try {
          ...
          if (translatedLiveReview.content.ar_SA) {
            updateData.content = mergeLocalizedTextWithArSA(
              currentLiveReview.content,
              translatedLiveReview.content.ar_SA,
            );
          }
