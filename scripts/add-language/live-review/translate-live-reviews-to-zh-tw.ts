...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-live-reviews-zh-tw.json');
 ...
 * 메인 번역 함수
 */
async function translateLiveReviewsToZhTW() {
  try {
    console.log('🌐 실시간 리뷰(LiveReview) 중국어 번체(zh-TW) 번역 작업 시작...');
    ...
    // 번역이 필요한 실시간 리뷰만 필터링 (zh_TW가 없거나 비어있는 경우)
    const liveReviewsToTranslate = allLiveReviews.filter((liveReview) => {
      ...
      // content 번역이 필요하면 포함
      return needsZhTWTranslation(liveReview.content);
    });

    const totalCount = liveReviewsToTranslate.length;
    console.log(
      `📊 총 ${allLiveReviews.length}개 실시간 리뷰 중 ${totalCount}개 실시간 리뷰가 중국어 번체 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const liveReview of liveReviews) {
        // content 번역
        if (needsZhTWTranslation(liveReview.content)) {
      ...
      for (const liveReview of liveReviews) {
        ...
        const translatedLiveReview: TranslatedLiveReview = {
          ...
          content: {
            ...
            th_TH: contentTh,
            zh_TW:
              translations.content_zh_TW ||
              getTextByLocale(liveReview.content, 'zh_TW') ||
              contentKo,
          },
        };
...
export { translateLiveReviewsToZhTW };
