...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-live-reviews-zh-tw.json');
 ...
 * 번역 결과 파일을 로드하는 함수
 */
function loadTranslationResults(): TranslatedLiveReview[] {
  try {
 ...
 * 메인 업데이트 함수
 */
async function updateLiveReviewsZhTW() {
  try {
    console.log('🔄 실시간 리뷰(LiveReview) 중국어 번체(zh-TW) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      // 배치별 업데이트 처리
      const updatePromises = batch.map(async (translatedLiveReview) => {
        try {
          // 현재 실시간 리뷰 데이터 조회
          const currentLiveReview = await prisma.liveReview.findUnique({
            where: { id: translatedLiveReview.id },
            select: {
              id: true,
              content: true,
            },
          ...
          if (!currentLiveReview) {
            console.warn(`⚠️  실시간 리뷰를 찾을 수 없습니다: ${translatedLiveReview.id}`);
            return {
              success: false,
              id: translatedLiveReview.id,
              error: 'LiveReview not found',
            };
          }
          ...
          if (translatedLiveReview.content.zh_TW) {
            updateData.content = mergeLocalizedTextWithZhTW(
              currentLiveReview.content,
              translatedLiveReview.content.zh_TW,
            );
          }
          ...
          return { success: true, id: translatedLiveReview.id };
        } catch (error) {
          console.error(`❌ 업데이트 실패 (${translatedLiveReview.id}):`, error);
          return {
            success: false,
            id: translatedLiveReview.id,
    ...
    console.log('\n🎉 모든 실시간 리뷰 중국어 번체(zh-TW) 업데이트 완료!');
    console.log(`📊 최종 결과:`);
