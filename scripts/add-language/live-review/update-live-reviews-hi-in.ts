...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-live-reviews-hi-in.json');
 ...
 * 메인 업데이트 함수
 */
async function updateLiveReviewsHiIN() {
  try {
    console.log('🔄 실시간 리뷰(LiveReview) 힌디어(hi-IN) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedLiveReview) => {
        try {
          ...
          if (translatedLiveReview.content.hi_IN) {
            updateData.content = mergeLocalizedTextWithHiIN(
              currentLiveReview.content,
              translatedLiveReview.content.hi_IN,
            );
          }
