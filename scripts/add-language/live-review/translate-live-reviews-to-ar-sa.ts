...
const OUTPUT_FILE = path.join(__dirname, 'output/translated-live-reviews-ar-sa.json');
...
async function translateLiveReviewsToArSA() {
  try {
    console.log('🌐 실시간 리뷰(LiveReview) 아랍어(ar-SA) 번역 작업 시작...');
    ...
    const liveReviewsToTranslate = allLiveReviews.filter((liveReview) => {
      ...
      return needsArSATranslation(liveReview.content);
    });

    const totalCount = liveReviewsToTranslate.length;
    console.log(
      `📊 총 ${allLiveReviews.length}개 실시간 리뷰 중 ${totalCount}개 실시간 리뷰가 아랍어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const liveReview of liveReviews) {
        if (needsArSATranslation(liveReview.content)) {
      ...
      const translationResults: { [key: string]: { content_ar_SA?: string } } = {};
        ...
        for (let i = 0; i < items.length; i += TRANSLATION_BATCH_SIZE) {
          ...
          try {
            ...
            for (const item of batch) {
              try {
                if (!translationResults[item.id]) translationResults[item.id] = {};
                translationResults[item.id].content_ar_SA = await translateText(
                  item.text,
                  item.sourceLang,
                  'ar',
                );
      ...
      for (const liveReview of liveReviews) {
        ...
        const translatedLiveReview: TranslatedLiveReview = {
          ...
          content: {
            ...
            hi_IN: contentHiIN,
            tl_PH: contentTlPH,
            ar_SA: tr.content_ar_SA || getTextByLocale(liveReview.content, 'ar_SA') || contentKo,
          },
        };
...
export { translateLiveReviewsToArSA };
