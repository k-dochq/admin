...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-hospitals-hi-in.json');
...
function needsHiINTranslation(localizedText: Prisma.JsonValue): boolean {
  ...
  if (
    ...
    // 소스 텍스트가 있고, hi_IN이 없거나 비어있으면 번역 필요
 ...
 * 메인 번역 함수
 */
async function translateHospitalsToHiIN() {
  try {
    console.log('🌐 병원 힌디어(hi-IN) 번역 작업 시작...');
    ...
    // 번역이 필요한 병원만 필터링 (hi_IN이 없거나 비어있는 경우)
    const hospitalsToTranslate = allHospitals.filter((hospital) => {
      ...
      return (
        needsHiINTranslation(hospital.name) ||
        needsHiINTranslation(hospital.address) ||
        (hospital.directions && needsHiINTranslation(hospital.directions)) ||
        (hospital.description && needsHiINTranslation(hospital.description)) ||
        (hospital.displayLocationName && needsHiINTranslation(hospital.displayLocationName))
      );
    });

    const totalCount = hospitalsToTranslate.length;
    console.log(
      `📊 총 ${allHospitals.length}개 병원 중 ${totalCount}개 병원이 힌디어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const hospital of hospitals) {
        // name 번역
        if (needsHiINTranslation(hospital.name)) {
        ...
        // address 번역
        if (needsHiINTranslation(hospital.address)) {
        ...
        // directions 번역
        if (hospital.directions && needsHiINTranslation(hospital.directions)) {
        ...
        // description 번역
        if (hospital.description && needsHiINTranslation(hospital.description)) {
        ...
        if (hospital.displayLocationName && needsHiINTranslation(hospital.displayLocationName)) {
      ...
      const translationResults: {
        [key: string]: {
          name_hi_IN?: string;
          address_hi_IN?: string;
          directions_hi_IN?: string;
          description_hi_IN?: string;
          displayLocationName_hi_IN?: string;
        };
      } = {};
      ...
      for (const [sourceLang, items] of Object.entries(textsBySourceLang)) {
        for (let i = 0; i < items.length; i += TRANSLATION_BATCH_SIZE) {
          ...
          try {
            console.log(
              `  🔄 힌디어 번역 중... (${sourceLang} → hi) (${i + 1}-${Math.min(i + TRANSLATION_BATCH_SIZE, items.length)}/${items.length})`,
            );
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].address_hi_IN = hiINTranslations[index];
              } else if (item.type === 'directions') {
                translationResults[item.id].directions_hi_IN = hiINTranslations[index];
              } else if (item.type === 'description') {
                translationResults[item.id].description_hi_IN = hiINTranslations[index];
              } else if (item.type === 'displayLocationName') {
                translationResults[item.id].displayLocationName_hi_IN = hiINTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].address_hi_IN = hiINTranslation;
                } else if (item.type === 'directions') {
                  translationResults[item.id].directions_hi_IN = hiINTranslation;
                } else if (item.type === 'description') {
                  translationResults[item.id].description_hi_IN = hiINTranslation;
                } else if (item.type === 'displayLocationName') {
                  translationResults[item.id].displayLocationName_hi_IN = hiINTranslation;
                }
      ...
      for (const hospital of hospitals) {
        ...
        const translatedHospital: TranslatedHospital = {
          ...
          name: {
            ...
            zh_TW: nameZhTW,
            ja_JP: nameJaJP,
            hi_IN: translations.name_hi_IN || getTextByLocale(hospital.name, 'hi_IN') || nameKo,
          },
          address: {
            ko_KR: addressKo,
            en_US: addressEn,
            ...
            ja_JP: addressJaJP,
            hi_IN:
              translations.address_hi_IN || getTextByLocale(hospital.address, 'hi_IN') || addressKo,
          },
        };
        ...
        if (hospital.directions) {
          ...
          translatedHospital.directions = {
            ...
            hi_IN:
              translations.directions_hi_IN ||
              getTextByLocale(hospital.directions, 'hi_IN') ||
              directionsKo,
          };
        }
        ...
        if (hospital.description) {
          ...
          translatedHospital.description = {
            ...
            hi_IN:
              translations.description_hi_IN ||
              getTextByLocale(hospital.description, 'hi_IN') ||
              descriptionKo,
          };
        }
        ...
        if (hospital.displayLocationName) {
          ...
          translatedHospital.displayLocationName = {
            ...
            hi_IN:
              translations.displayLocationName_hi_IN ||
              getTextByLocale(hospital.displayLocationName, 'hi_IN') ||
...
export { translateHospitalsToHiIN };
