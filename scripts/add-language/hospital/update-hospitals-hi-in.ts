...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-hospitals-hi-in.json');
 ...
 * 메인 업데이트 함수
 */
async function updateHospitalsHiIN() {
  try {
    console.log('🔄 병원 힌디어(hi-IN) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedHospital) => {
        try {
          ...
          if (translatedHospital.name.hi_IN) {
            updateData.name = mergeLocalizedTextWithHiIN(
              currentHospital.name,
              translatedHospital.name.hi_IN,
            );
          }

          // address 업데이트 (기존 데이터 유지하고 hi_IN만 추가)
          if (translatedHospital.address.hi_IN) {
            updateData.address = mergeLocalizedTextWithHiIN(
              currentHospital.address,
              translatedHospital.address.hi_IN,
            );
          }

          // directions 업데이트 (기존 데이터 유지하고 hi_IN만 추가)
          if (translatedHospital.directions && translatedHospital.directions.hi_IN) {
            updateData.directions = mergeLocalizedTextWithHiIN(
              currentHospital.directions,
              translatedHospital.directions.hi_IN,
            );
          }

          // description 업데이트 (기존 데이터 유지하고 hi_IN만 추가)
          if (translatedHospital.description && translatedHospital.description.hi_IN) {
            updateData.description = mergeLocalizedTextWithHiIN(
              currentHospital.description,
              translatedHospital.description.hi_IN,
            );
          }

          // displayLocationName 업데이트 (기존 데이터 유지하고 hi_IN만 추가)
          if (
            translatedHospital.displayLocationName &&
            translatedHospital.displayLocationName.hi_IN
          ) {
            updateData.displayLocationName = mergeLocalizedTextWithHiIN(
