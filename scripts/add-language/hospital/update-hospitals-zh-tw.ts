...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-hospitals-zh-tw.json');
 ...
 * 메인 업데이트 함수
 */
async function updateHospitalsZhTW() {
  try {
    console.log('🔄 병원 중국어 번체(zh-TW) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedHospital) => {
        try {
          ...
          if (translatedHospital.name.zh_TW) {
            updateData.name = mergeLocalizedTextWithZhTW(
              currentHospital.name,
              translatedHospital.name.zh_TW,
            );
          }

          // address 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedHospital.address.zh_TW) {
            updateData.address = mergeLocalizedTextWithZhTW(
              currentHospital.address,
              translatedHospital.address.zh_TW,
            );
          }

          // directions 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedHospital.directions && translatedHospital.directions.zh_TW) {
            updateData.directions = mergeLocalizedTextWithZhTW(
              currentHospital.directions,
              translatedHospital.directions.zh_TW,
            );
          }

          // description 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedHospital.description && translatedHospital.description.zh_TW) {
            updateData.description = mergeLocalizedTextWithZhTW(
              currentHospital.description,
              translatedHospital.description.zh_TW,
            );
          }

          // displayLocationName 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (
            translatedHospital.displayLocationName &&
            translatedHospital.displayLocationName.zh_TW
          ) {
            updateData.displayLocationName = mergeLocalizedTextWithZhTW(
