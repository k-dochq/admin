...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-hospitals-ja-jp.json');
 ...
 * 메인 번역 함수
 */
async function translateHospitalsToJaJP() {
  try {
    console.log('🌐 병원 일본어(ja-JP) 번역 작업 시작...');
    ...
    // 번역이 필요한 병원만 필터링 (ja_JP가 없거나 비어있는 경우)
    const hospitalsToTranslate = allHospitals.filter((hospital) => {
      ...
      return (
        needsJaJPTranslation(hospital.name) ||
        needsJaJPTranslation(hospital.address) ||
        (hospital.directions && needsJaJPTranslation(hospital.directions)) ||
        (hospital.description && needsJaJPTranslation(hospital.description)) ||
        (hospital.displayLocationName && needsJaJPTranslation(hospital.displayLocationName))
      );
    });

    const totalCount = hospitalsToTranslate.length;
    console.log(
      `📊 총 ${allHospitals.length}개 병원 중 ${totalCount}개 병원이 일본어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const hospital of hospitals) {
        // name 번역
        if (needsJaJPTranslation(hospital.name)) {
        ...
        // address 번역
        if (needsJaJPTranslation(hospital.address)) {
        ...
        // directions 번역
        if (hospital.directions && needsJaJPTranslation(hospital.directions)) {
        ...
        // description 번역
        if (hospital.description && needsJaJPTranslation(hospital.description)) {
        ...
        if (hospital.displayLocationName && needsJaJPTranslation(hospital.displayLocationName)) {
      ...
      const translationResults: {
        [key: string]: {
          name_ja_JP?: string;
          address_ja_JP?: string;
          directions_ja_JP?: string;
          description_ja_JP?: string;
          displayLocationName_ja_JP?: string;
        };
      } = {};
        ...
        for (let i = 0; i < items.length; i += TRANSLATION_BATCH_SIZE) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                translationResults[item.id].name_ja_JP = jaJPTranslations[index];
              } else if (item.type === 'address') {
                translationResults[item.id].address_ja_JP = jaJPTranslations[index];
              } else if (item.type === 'directions') {
                translationResults[item.id].directions_ja_JP = jaJPTranslations[index];
              } else if (item.type === 'description') {
                translationResults[item.id].description_ja_JP = jaJPTranslations[index];
              } else if (item.type === 'displayLocationName') {
                translationResults[item.id].displayLocationName_ja_JP = jaJPTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  translationResults[item.id].name_ja_JP = jaJPTranslation;
                } else if (item.type === 'address') {
                  translationResults[item.id].address_ja_JP = jaJPTranslation;
                } else if (item.type === 'directions') {
                  translationResults[item.id].directions_ja_JP = jaJPTranslation;
                } else if (item.type === 'description') {
                  translationResults[item.id].description_ja_JP = jaJPTranslation;
                } else if (item.type === 'displayLocationName') {
                  translationResults[item.id].displayLocationName_ja_JP = jaJPTranslation;
                }
      ...
      for (const hospital of hospitals) {
        ...
        const translatedHospital: TranslatedHospital = {
          ...
          name: {
            ...
            th_TH: nameTh,
            zh_TW: nameZhTW,
            ja_JP: translations.name_ja_JP || getTextByLocale(hospital.name, 'ja_JP') || nameKo,
          },
          address: {
            ko_KR: addressKo,
            en_US: addressEn,
            ...
            zh_TW: addressZhTW,
            ja_JP:
              translations.address_ja_JP || getTextByLocale(hospital.address, 'ja_JP') || addressKo,
          },
        };
        ...
        if (hospital.directions) {
          ...
          translatedHospital.directions = {
            ...
            ja_JP:
              translations.directions_ja_JP ||
              getTextByLocale(hospital.directions, 'ja_JP') ||
              directionsKo,
          };
        }
        ...
        if (hospital.description) {
          ...
          translatedHospital.description = {
            ...
            ja_JP:
              translations.description_ja_JP ||
              getTextByLocale(hospital.description, 'ja_JP') ||
              descriptionKo,
          };
        }
        ...
        if (hospital.displayLocationName) {
          ...
          translatedHospital.displayLocationName = {
            ...
            ja_JP:
              translations.displayLocationName_ja_JP ||
              getTextByLocale(hospital.displayLocationName, 'ja_JP') ||
...
export { translateHospitalsToJaJP };
