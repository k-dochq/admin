...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-hospitals-zh-tw.json');
 ...
 * 메인 번역 함수
 */
async function translateHospitalsToZhTW() {
  try {
    console.log('🌐 병원 중국어 번체(zh-TW) 번역 작업 시작...');
    ...
    // 번역이 필요한 병원만 필터링 (zh_TW가 없거나 비어있는 경우)
    const hospitalsToTranslate = allHospitals.filter((hospital) => {
      ...
      return (
        needsZhTWTranslation(hospital.name) ||
        needsZhTWTranslation(hospital.address) ||
        (hospital.directions && needsZhTWTranslation(hospital.directions)) ||
        (hospital.description && needsZhTWTranslation(hospital.description)) ||
        (hospital.displayLocationName && needsZhTWTranslation(hospital.displayLocationName))
      );
    });

    const totalCount = hospitalsToTranslate.length;
    console.log(
      `📊 총 ${allHospitals.length}개 병원 중 ${totalCount}개 병원이 중국어 번체 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const hospital of hospitals) {
        // name 번역
        if (needsZhTWTranslation(hospital.name)) {
        ...
        // address 번역
        if (needsZhTWTranslation(hospital.address)) {
        ...
        // directions 번역
        if (hospital.directions && needsZhTWTranslation(hospital.directions)) {
        ...
        // description 번역
        if (hospital.description && needsZhTWTranslation(hospital.description)) {
        ...
        if (hospital.displayLocationName && needsZhTWTranslation(hospital.displayLocationName)) {
      ...
      const translationResults: {
        [key: string]: {
          ...
          directions_zh_TW?: string;
          description_zh_TW?: string;
          displayLocationName_zh_TW?: string;
        };
      } = {};
        ...
        for (let i = 0; i < items.length; i += TRANSLATION_BATCH_SIZE) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                translationResults[item.id].name_zh_TW = zhTWTranslations[index];
              } else if (item.type === 'address') {
                translationResults[item.id].address_zh_TW = zhTWTranslations[index];
              } else if (item.type === 'directions') {
                translationResults[item.id].directions_zh_TW = zhTWTranslations[index];
              } else if (item.type === 'description') {
                translationResults[item.id].description_zh_TW = zhTWTranslations[index];
              } else if (item.type === 'displayLocationName') {
                translationResults[item.id].displayLocationName_zh_TW = zhTWTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  translationResults[item.id].name_zh_TW = zhTWTranslation;
                } else if (item.type === 'address') {
                  translationResults[item.id].address_zh_TW = zhTWTranslation;
                } else if (item.type === 'directions') {
                  translationResults[item.id].directions_zh_TW = zhTWTranslation;
                } else if (item.type === 'description') {
                  translationResults[item.id].description_zh_TW = zhTWTranslation;
                } else if (item.type === 'displayLocationName') {
                  translationResults[item.id].displayLocationName_zh_TW = zhTWTranslation;
                }
      ...
      for (const hospital of hospitals) {
        ...
        const translatedHospital: TranslatedHospital = {
          ...
          name: {
            ...
            en_US: nameEn,
            th_TH: nameTh,
            zh_TW: translations.name_zh_TW || getTextByLocale(hospital.name, 'zh_TW') || nameKo,
          },
          address: {
            ko_KR: addressKo,
            en_US: addressEn,
            th_TH: addressTh,
            zh_TW:
              translations.address_zh_TW || getTextByLocale(hospital.address, 'zh_TW') || addressKo,
          },
        };
        ...
        if (hospital.directions) {
          ...
          translatedHospital.directions = {
            ...
            th_TH: directionsTh,
            zh_TW:
              translations.directions_zh_TW ||
              getTextByLocale(hospital.directions, 'zh_TW') ||
              directionsKo,
          };
        }
        ...
        if (hospital.description) {
          ...
          translatedHospital.description = {
            ...
            th_TH: descriptionTh,
            zh_TW:
              translations.description_zh_TW ||
              getTextByLocale(hospital.description, 'zh_TW') ||
              descriptionKo,
          };
        }
        ...
        if (hospital.displayLocationName) {
          ...
          translatedHospital.displayLocationName = {
            ...
            zh_TW:
              translations.displayLocationName_zh_TW ||
              getTextByLocale(hospital.displayLocationName, 'zh_TW') ||
...
export { translateHospitalsToZhTW };
