...
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-hospitals-ar-sa.json');
...
async function updateHospitalsArSA() {
  try {
    console.log('🔄 병원 아랍어(ar-SA) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedHospital) => {
        try {
          ...
          if (translatedHospital.name.ar_SA) {
            updateData.name = mergeLocalizedTextWithArSA(
              currentHospital.name,
              translatedHospital.name.ar_SA,
            );
          }
          if (translatedHospital.address.ar_SA) {
            updateData.address = mergeLocalizedTextWithArSA(
              currentHospital.address,
              translatedHospital.address.ar_SA,
            );
          }
          if (translatedHospital.directions?.ar_SA) {
            updateData.directions = mergeLocalizedTextWithArSA(
              currentHospital.directions,
              translatedHospital.directions.ar_SA,
            );
          }
          if (translatedHospital.description?.ar_SA) {
            updateData.description = mergeLocalizedTextWithArSA(
              currentHospital.description,
              translatedHospital.description.ar_SA,
            );
          }
          if (translatedHospital.displayLocationName?.ar_SA) {
            updateData.displayLocationName = mergeLocalizedTextWithArSA(
              currentHospital.displayLocationName,
              translatedHospital.displayLocationName.ar_SA,
            );
          }
