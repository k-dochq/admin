...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-hospitals-ja-jp.json');
 ...
 * 메인 업데이트 함수
 */
async function updateHospitalsJaJP() {
  try {
    console.log('🔄 병원 일본어(ja-JP) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedHospital) => {
        try {
          ...
          if (translatedHospital.name.ja_JP) {
            updateData.name = mergeLocalizedTextWithJaJP(
              currentHospital.name,
              translatedHospital.name.ja_JP,
            );
          }

          // address 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (translatedHospital.address.ja_JP) {
            updateData.address = mergeLocalizedTextWithJaJP(
              currentHospital.address,
              translatedHospital.address.ja_JP,
            );
          }

          // directions 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (translatedHospital.directions && translatedHospital.directions.ja_JP) {
            updateData.directions = mergeLocalizedTextWithJaJP(
              currentHospital.directions,
              translatedHospital.directions.ja_JP,
            );
          }

          // description 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (translatedHospital.description && translatedHospital.description.ja_JP) {
            updateData.description = mergeLocalizedTextWithJaJP(
              currentHospital.description,
              translatedHospital.description.ja_JP,
            );
          }

          // displayLocationName 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (
            translatedHospital.displayLocationName &&
            translatedHospital.displayLocationName.ja_JP
          ) {
            updateData.displayLocationName = mergeLocalizedTextWithJaJP(
