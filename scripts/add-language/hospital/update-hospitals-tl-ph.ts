...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-hospitals-tl-ph.json');
 ...
 * 메인 업데이트 함수
 */
async function updateHospitalsTlPH() {
  try {
    console.log('🔄 병원 필리핀어(tl-PH) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedHospital) => {
        try {
          ...
          if (translatedHospital.name.tl_PH) {
            updateData.name = mergeLocalizedTextWithTlPH(
              currentHospital.name,
              translatedHospital.name.tl_PH,
            );
          }

          // address 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedHospital.address.tl_PH) {
            updateData.address = mergeLocalizedTextWithTlPH(
              currentHospital.address,
              translatedHospital.address.tl_PH,
            );
          }

          // directions 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedHospital.directions && translatedHospital.directions.tl_PH) {
            updateData.directions = mergeLocalizedTextWithTlPH(
              currentHospital.directions,
              translatedHospital.directions.tl_PH,
            );
          }

          // description 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedHospital.description && translatedHospital.description.tl_PH) {
            updateData.description = mergeLocalizedTextWithTlPH(
              currentHospital.description,
              translatedHospital.description.tl_PH,
            );
          }

          // displayLocationName 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (
            translatedHospital.displayLocationName &&
            translatedHospital.displayLocationName.tl_PH
          ) {
            updateData.displayLocationName = mergeLocalizedTextWithTlPH(
