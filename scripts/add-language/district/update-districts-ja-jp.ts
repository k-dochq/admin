...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-districts-ja-jp.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDistrictsJaJP() {
  try {
    console.log('🔄 지역(District) 일본어(ja-JP) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDistrict) => {
        try {
          ...
          if (translatedDistrict.name.ja_JP) {
            updateData.name = mergeLocalizedTextWithJaJP(
              currentDistrict.name,
              translatedDistrict.name.ja_JP,
            );
          }

          // displayName 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (translatedDistrict.displayName && translatedDistrict.displayName.ja_JP) {
            updateData.displayName = mergeLocalizedTextWithJaJP(
              currentDistrict.displayName,
              translatedDistrict.displayName.ja_JP,
            );
          }
