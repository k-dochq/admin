...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-districts-zh-tw.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDistrictsZhTW() {
  try {
    console.log('🔄 지역(District) 중국어 번체(zh-TW) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDistrict) => {
        try {
          ...
          if (translatedDistrict.name.zh_TW) {
            updateData.name = mergeLocalizedTextWithZhTW(
              currentDistrict.name,
              translatedDistrict.name.zh_TW,
            );
          }

          // displayName 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedDistrict.displayName && translatedDistrict.displayName.zh_TW) {
            updateData.displayName = mergeLocalizedTextWithZhTW(
              currentDistrict.displayName,
              translatedDistrict.displayName.zh_TW,
            );
          }
