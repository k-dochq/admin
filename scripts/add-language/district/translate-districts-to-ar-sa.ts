...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-districts-ar-sa.json');
 ...
 * 메인 번역 함수
 */
async function translateDistrictsToArSA() {
  try {
    console.log('🌐 지역(District) 아랍어(ar-SA) 번역 작업 시작...');
    ...
    // 번역이 필요한 지역만 필터링 (ar_SA가 없거나 비어있는 경우)
    const districtsToTranslate = allDistricts.filter((district) => {
      ...
      return (
        needsArSATranslation(district.name) ||
        (district.displayName && needsArSATranslation(district.displayName))
      );
    });

    const totalCount = districtsToTranslate.length;
    console.log(
      `📊 총 ${allDistricts.length}개 지역 중 ${totalCount}개 지역이 아랍어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const district of districts) {
        // name 번역
        if (needsArSATranslation(district.name)) {
        ...
        // displayName 번역
        if (district.displayName && needsArSATranslation(district.displayName)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].displayName_ar_SA = arSATranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].displayName_ar_SA = arSATranslation;
                }
      ...
      for (const district of districts) {
        ...
        const translatedDistrict: TranslatedDistrict = {
          ...
          name: {
            ...
            hi_IN: nameHiIN,
            tl_PH: nameTlPH,
            ar_SA: translations.name_ar_SA || getTextByLocale(district.name, 'ar_SA') || nameKo,
          },
        };
        ...
        if (district.displayName) {
          ...
          translatedDistrict.displayName = {
            ...
            ar_SA:
              translations.displayName_ar_SA ||
              getTextByLocale(district.displayName, 'ar_SA') ||
              displayNameKo,
          };
        }
...
export { translateDistrictsToArSA };
