...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-districts-tl-ph.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDistrictsTlPH() {
  try {
    console.log('🔄 지역(District) 필리핀어(tl-PH) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDistrict) => {
        try {
          ...
          if (translatedDistrict.name.tl_PH) {
            updateData.name = mergeLocalizedTextWithTlPH(
              currentDistrict.name,
              translatedDistrict.name.tl_PH,
            );
          }

          // displayName 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedDistrict.displayName && translatedDistrict.displayName.tl_PH) {
            updateData.displayName = mergeLocalizedTextWithTlPH(
              currentDistrict.displayName,
              translatedDistrict.displayName.tl_PH,
            );
          }
