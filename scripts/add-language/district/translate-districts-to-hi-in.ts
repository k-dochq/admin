...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-districts-hi-in.json');
 ...
 * 메인 번역 함수
 */
async function translateDistrictsToHiIN() {
  try {
    console.log('🌐 지역(District) 힌디어(hi-IN) 번역 작업 시작...');
    ...
    // 번역이 필요한 지역만 필터링 (hi_IN이 없거나 비어있는 경우)
    const districtsToTranslate = allDistricts.filter((district) => {
      ...
      return (
        needsHiINTranslation(district.name) ||
        (district.displayName && needsHiINTranslation(district.displayName))
      );
    });

    const totalCount = districtsToTranslate.length;
    console.log(
      `📊 총 ${allDistricts.length}개 지역 중 ${totalCount}개 지역이 힌디어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const district of districts) {
        // name 번역
        if (needsHiINTranslation(district.name)) {
        ...
        // displayName 번역
        if (district.displayName && needsHiINTranslation(district.displayName)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].displayName_hi_IN = hiINTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].displayName_hi_IN = hiINTranslation;
                }
      ...
      for (const district of districts) {
        ...
        const translatedDistrict: TranslatedDistrict = {
          ...
          name: {
            ...
            zh_TW: nameZhTW,
            ja_JP: nameJaJP,
            hi_IN: translations.name_hi_IN || getTextByLocale(district.name, 'hi_IN') || nameKo,
          },
        };
        ...
        if (district.displayName) {
          ...
          translatedDistrict.displayName = {
            ...
            hi_IN:
              translations.displayName_hi_IN ||
              getTextByLocale(district.displayName, 'hi_IN') ||
              displayNameKo,
          };
        }
...
export { translateDistrictsToHiIN };
