...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-districts-ja-jp.json');
 ...
 * 메인 번역 함수
 */
async function translateDistrictsToJaJP() {
  try {
    console.log('🌐 지역(District) 일본어(ja-JP) 번역 작업 시작...');
    ...
    // 번역이 필요한 지역만 필터링 (ja_JP가 없거나 비어있는 경우)
    const districtsToTranslate = allDistricts.filter((district) => {
      ...
      return (
        needsJaJPTranslation(district.name) ||
        (district.displayName && needsJaJPTranslation(district.displayName))
      );
    });

    const totalCount = districtsToTranslate.length;
    console.log(
      `📊 총 ${allDistricts.length}개 지역 중 ${totalCount}개 지역이 일본어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const district of districts) {
        // name 번역
        if (needsJaJPTranslation(district.name)) {
        ...
        // displayName 번역
        if (district.displayName && needsJaJPTranslation(district.displayName)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].displayName_ja_JP = jaJPTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].displayName_ja_JP = jaJPTranslation;
                }
      ...
      for (const district of districts) {
        ...
        const translatedDistrict: TranslatedDistrict = {
          ...
          name: {
            ...
            th_TH: nameTh,
            zh_TW: nameZhTW,
            ja_JP: translations.name_ja_JP || getTextByLocale(district.name, 'ja_JP') || nameKo,
          },
        };
        ...
        if (district.displayName) {
          ...
          translatedDistrict.displayName = {
            ...
            ja_JP:
              translations.displayName_ja_JP ||
              getTextByLocale(district.displayName, 'ja_JP') ||
              displayNameKo,
          };
        }
...
export { translateDistrictsToJaJP };
