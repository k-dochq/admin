...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-districts-hi-in.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDistrictsHiIN() {
  try {
    console.log('🔄 지역(District) 힌디어(hi-IN) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDistrict) => {
        try {
          ...
          if (translatedDistrict.name.hi_IN) {
            updateData.name = mergeLocalizedTextWithHiIN(
              currentDistrict.name,
              translatedDistrict.name.hi_IN,
            );
          }

          // displayName 업데이트 (기존 데이터 유지하고 hi_IN만 추가)
          if (translatedDistrict.displayName && translatedDistrict.displayName.hi_IN) {
            updateData.displayName = mergeLocalizedTextWithHiIN(
              currentDistrict.displayName,
              translatedDistrict.displayName.hi_IN,
            );
          }
