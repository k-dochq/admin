...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-districts-zh-tw.json');
 ...
 * 메인 번역 함수
 */
async function translateDistrictsToZhTW() {
  try {
    console.log('🌐 지역(District) 중국어 번체(zh-TW) 번역 작업 시작...');
    ...
    // 번역이 필요한 지역만 필터링 (zh_TW가 없거나 비어있는 경우)
    const districtsToTranslate = allDistricts.filter((district) => {
      ...
      return (
        needsZhTWTranslation(district.name) ||
        (district.displayName && needsZhTWTranslation(district.displayName))
      );
    });

    const totalCount = districtsToTranslate.length;
    console.log(
      `📊 총 ${allDistricts.length}개 지역 중 ${totalCount}개 지역이 중국어 번체 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const district of districts) {
        // name 번역
        if (needsZhTWTranslation(district.name)) {
        ...
        // displayName 번역
        if (district.displayName && needsZhTWTranslation(district.displayName)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].displayName_zh_TW = zhTWTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].displayName_zh_TW = zhTWTranslation;
                }
      ...
      for (const district of districts) {
        ...
        const translatedDistrict: TranslatedDistrict = {
          ...
          name: {
            ...
            en_US: nameEn,
            th_TH: nameTh,
            zh_TW: translations.name_zh_TW || getTextByLocale(district.name, 'zh_TW') || nameKo,
          },
        };
        ...
        if (district.displayName) {
          ...
          translatedDistrict.displayName = {
            ...
            th_TH: displayNameTh,
            zh_TW:
              translations.displayName_zh_TW ||
              getTextByLocale(district.displayName, 'zh_TW') ||
              displayNameKo,
          };
        }
...
export { translateDistrictsToZhTW };
