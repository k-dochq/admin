...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-districts-ru-ru.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDistrictsRuRU() {
  try {
    console.log('🔄 지역(District) 러시아어(ru-RU) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDistrict) => {
        try {
          ...
          if (translatedDistrict.name.ru_RU) {
            updateData.name = mergeLocalizedTextWithRuRU(
              currentDistrict.name,
              translatedDistrict.name.ru_RU,
            );
          }

          // displayName 업데이트 (기존 데이터 유지하고 ru_RU만 추가)
          if (translatedDistrict.displayName && translatedDistrict.displayName.ru_RU) {
            updateData.displayName = mergeLocalizedTextWithRuRU(
              currentDistrict.displayName,
              translatedDistrict.displayName.ru_RU,
            );
          }
