...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-reviews-ja-jp.json');
 ...
 * 메인 업데이트 함수
 */
async function updateReviewsJaJP() {
  try {
    console.log('🔄 리뷰 일본어(ja-JP) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedReview) => {
        try {
          ...
          if (translatedReview.title.ja_JP) {
            updateData.title = mergeLocalizedTextWithJaJP(
              currentReview.title,
              translatedReview.title.ja_JP,
            );
          }

          // content 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (translatedReview.content.ja_JP) {
            updateData.content = mergeLocalizedTextWithJaJP(
              currentReview.content,
              translatedReview.content.ja_JP,
            );
          }

          // concernsMultilingual 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (
            translatedReview.concernsMultilingual &&
            translatedReview.concernsMultilingual.ja_JP
          ) {
            updateData.concernsMultilingual = mergeLocalizedTextWithJaJP(
