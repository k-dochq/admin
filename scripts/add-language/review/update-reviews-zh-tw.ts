...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-reviews-zh-tw.json');
const UPDATE_PROGRESS_FILE = path.join(__dirname, 'output/update-progress-zh-tw.json');
...
/**
 * 기존 LocalizedText에 zh_TW만 추가/업데이트하는 함수
 */
function mergeLocalizedTextWithZhTW(existing: Prisma.JsonValue, zhTWText: string): LocalizedText {
 ...
 * 번역 결과 파일을 로드하는 함수
 */
function loadTranslationResults(): TranslatedReview[] {
  try {
 ...
 * 메인 업데이트 함수
 */
async function updateReviewsZhTW() {
  try {
    console.log('🔄 리뷰 중국어 번체(zh-TW) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      // 배치별 업데이트 처리
      const updatePromises = batch.map(async (translatedReview) => {
        try {
          // 현재 리뷰 데이터 조회
          const currentReview = await prisma.review.findUnique({
            where: { id: translatedReview.id },
            select: {
              id: true,
              title: true,
              content: true,
          ...
          if (!currentReview) {
            console.warn(`⚠️  리뷰를 찾을 수 없습니다: ${translatedReview.id}`);
            return { success: false, id: translatedReview.id, error: 'Review not found' };
          }
          ...
          const updateData: {
            ...
            concernsMultilingual?: LocalizedText;
          } = {};

          // title 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedReview.title.zh_TW) {
            updateData.title = mergeLocalizedTextWithZhTW(
              currentReview.title,
              translatedReview.title.zh_TW,
            );
          }

          // content 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedReview.content.zh_TW) {
            updateData.content = mergeLocalizedTextWithZhTW(
              currentReview.content,
              translatedReview.content.zh_TW,
            );
          }

          // concernsMultilingual 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (
            translatedReview.concernsMultilingual &&
            translatedReview.concernsMultilingual.zh_TW
          ) {
            updateData.concernsMultilingual = mergeLocalizedTextWithZhTW(
              currentReview.concernsMultilingual,
              translatedReview.concernsMultilingual.zh_TW,
            );
          }
          ...
          return { success: true, id: translatedReview.id };
        } catch (error) {
          console.error(`❌ 업데이트 실패 (${translatedReview.id}):`, error);
          return {
            success: false,
            id: translatedReview.id,
