...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-reviews-ar-sa.json');
 ...
 * 메인 업데이트 함수
 */
async function updateReviewsArSA() {
  try {
    console.log('🔄 리뷰 아랍어(ar-SA) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedReview) => {
        try {
          ...
          // title 업데이트 (기존 데이터 유지하고 ar_SA만 추가)
          if (translatedReview.title.ar_SA) {
            updateData.title = mergeLocalizedTextWithArSA(
              currentReview.title,
              translatedReview.title.ar_SA,
            );
          }

          // content 업데이트 (기존 데이터 유지하고 ar_SA만 추가)
          if (translatedReview.content.ar_SA) {
            updateData.content = mergeLocalizedTextWithArSA(
              currentReview.content,
              translatedReview.content.ar_SA,
            );
          }

          // concernsMultilingual 업데이트 (기존 데이터 유지하고 ar_SA만 추가)
          if (
            translatedReview.concernsMultilingual &&
            translatedReview.concernsMultilingual.ar_SA
          ) {
            updateData.concernsMultilingual = mergeLocalizedTextWithArSA(
