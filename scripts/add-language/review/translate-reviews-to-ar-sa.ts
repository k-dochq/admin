...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-reviews-ar-sa.json');
 ...
 * 메인 번역 함수
 */
async function translateReviewsToArSA() {
  try {
    console.log('🌐 리뷰 아랍어(ar-SA) 번역 작업 시작...');
    ...
    // 번역이 필요한 리뷰만 필터링 (ar_SA가 없거나 비어있는 경우)
    const reviewsToTranslate = allReviews.filter((review) => {
      ...
      return (
        needsArSATranslation(review.title) ||
        needsArSATranslation(review.content) ||
        (review.concernsMultilingual && needsArSATranslation(review.concernsMultilingual))
      );
    });

    const totalCount = reviewsToTranslate.length;
    console.log(
      `📊 총 ${allReviews.length}개 리뷰 중 ${totalCount}개 리뷰가 아랍어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const review of reviews) {
        // 제목 번역
        if (needsArSATranslation(review.title)) {
        ...
        // 내용 번역
        if (needsArSATranslation(review.content)) {
        ...
        if (review.concernsMultilingual && needsArSATranslation(review.concernsMultilingual)) {
      ...
      const translationResults: {
        [key: string]: {
          title_ar_SA?: string;
          content_ar_SA?: string;
          concerns_ar_SA?: string;
        };
      } = {};
        ...
        for (let i = 0; i < items.length; i += TRANSLATION_BATCH_SIZE) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'title') {
                translationResults[item.id].title_ar_SA = arSATranslations[index];
              } else if (item.type === 'content') {
                translationResults[item.id].content_ar_SA = arSATranslations[index];
              } else if (item.type === 'concerns') {
                translationResults[item.id].concerns_ar_SA = arSATranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'title') {
                  translationResults[item.id].title_ar_SA = arSATranslation;
                } else if (item.type === 'content') {
                  translationResults[item.id].content_ar_SA = arSATranslation;
                } else if (item.type === 'concerns') {
                  translationResults[item.id].concerns_ar_SA = arSATranslation;
                }
      ...
      for (const review of reviews) {
        ...
        const translatedReview: TranslatedReview = {
          ...
          title: {
            ...
            hi_IN: titleHiIN,
            tl_PH: titleTlPH,
            ar_SA: translations.title_ar_SA || getTextByLocale(review.title, 'ar_SA') || titleKo,
          },
          content: {
            ko_KR: contentKo,
            en_US: contentEn,
            ...
            tl_PH: contentTlPH,
            ar_SA:
              translations.content_ar_SA || getTextByLocale(review.content, 'ar_SA') || contentKo,
          },
        };
        ...
        if (review.concernsMultilingual) {
          ...
          translatedReview.concernsMultilingual = {
            ...
            tl_PH: concernsTlPH,
            ar_SA:
              translations.concerns_ar_SA ||
              getTextByLocale(review.concernsMultilingual, 'ar_SA') ||
              concernsKo,
          };
        }
