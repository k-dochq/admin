...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-reviews-hi-in.json');
 ...
 * 메인 번역 함수
 */
async function translateReviewsToHiIN() {
  try {
    console.log('🌐 리뷰 힌디어(hi-IN) 번역 작업 시작...');
    ...
    // 번역이 필요한 리뷰만 필터링 (hi_IN이 없거나 비어있는 경우)
    const reviewsToTranslate = allReviews.filter((review) => {
      ...
      return (
        needsHiINTranslation(review.title) ||
        needsHiINTranslation(review.content) ||
        (review.concernsMultilingual && needsHiINTranslation(review.concernsMultilingual))
      );
    });

    const totalCount = reviewsToTranslate.length;
    console.log(
      `📊 총 ${allReviews.length}개 리뷰 중 ${totalCount}개 리뷰가 힌디어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const review of reviews) {
        // 제목 번역
        if (needsHiINTranslation(review.title)) {
        ...
        // 내용 번역
        if (needsHiINTranslation(review.content)) {
        ...
        if (review.concernsMultilingual && needsHiINTranslation(review.concernsMultilingual)) {
      ...
      const translationResults: {
        [key: string]: {
          title_hi_IN?: string;
          content_hi_IN?: string;
          concerns_hi_IN?: string;
        };
      } = {};
        ...
        for (let i = 0; i < items.length; i += TRANSLATION_BATCH_SIZE) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'title') {
                translationResults[item.id].title_hi_IN = hiINTranslations[index];
              } else if (item.type === 'content') {
                translationResults[item.id].content_hi_IN = hiINTranslations[index];
              } else if (item.type === 'concerns') {
                translationResults[item.id].concerns_hi_IN = hiINTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'title') {
                  translationResults[item.id].title_hi_IN = hiINTranslation;
                } else if (item.type === 'content') {
                  translationResults[item.id].content_hi_IN = hiINTranslation;
                } else if (item.type === 'concerns') {
                  translationResults[item.id].concerns_hi_IN = hiINTranslation;
                }
      ...
      for (const review of reviews) {
        ...
        const translatedReview: TranslatedReview = {
          ...
          title: {
            ...
            zh_TW: titleZhTW,
            ja_JP: titleJaJP,
            hi_IN: translations.title_hi_IN || getTextByLocale(review.title, 'hi_IN') || titleKo,
          },
          content: {
            ko_KR: contentKo,
            en_US: contentEn,
            ...
            ja_JP: contentJaJP,
            hi_IN:
              translations.content_hi_IN || getTextByLocale(review.content, 'hi_IN') || contentKo,
          },
        };
        ...
        if (review.concernsMultilingual) {
          ...
          translatedReview.concernsMultilingual = {
            ...
            ja_JP: concernsJaJP,
            hi_IN:
              translations.concerns_hi_IN ||
              getTextByLocale(review.concernsMultilingual, 'hi_IN') ||
              concernsKo,
          };
        }
