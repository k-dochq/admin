...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-reviews-tl-ph.json');
const UPDATE_PROGRESS_FILE = path.join(__dirname, 'output/update-progress-tl-ph.json');
...
/**
 * 기존 LocalizedText에 tl_PH만 추가/업데이트하는 함수
 */
function mergeLocalizedTextWithTlPH(existing: Prisma.JsonValue, tlPHText: string): LocalizedText {
 ...
 * 메인 업데이트 함수
 */
async function updateReviewsTlPH() {
  try {
    console.log('🔄 리뷰 필리핀어(tl-PH) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedReview) => {
        try {
          ...
          // title 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedReview.title.tl_PH) {
            updateData.title = mergeLocalizedTextWithTlPH(
              currentReview.title,
              translatedReview.title.tl_PH,
            );
          }

          // content 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedReview.content.tl_PH) {
            updateData.content = mergeLocalizedTextWithTlPH(
              currentReview.content,
              translatedReview.content.tl_PH,
            );
          }

          // concernsMultilingual 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (
            translatedReview.concernsMultilingual &&
            translatedReview.concernsMultilingual.tl_PH
          ) {
            updateData.concernsMultilingual = mergeLocalizedTextWithTlPH(
