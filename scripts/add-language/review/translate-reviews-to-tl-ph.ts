...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-reviews-tl-ph.json');
 ...
 * 메인 번역 함수
 */
async function translateReviewsToTlPH() {
  try {
    console.log('🌐 리뷰 필리핀어(tl-PH) 번역 작업 시작...');
    ...
    // 번역이 필요한 리뷰만 필터링 (tl_PH가 없거나 비어있는 경우)
    const reviewsToTranslate = allReviews.filter((review) => {
      ...
      return (
        needsTlPHTranslation(review.title) ||
        needsTlPHTranslation(review.content) ||
        (review.concernsMultilingual && needsTlPHTranslation(review.concernsMultilingual))
      );
    });

    const totalCount = reviewsToTranslate.length;
    console.log(
      `📊 총 ${allReviews.length}개 리뷰 중 ${totalCount}개 리뷰가 필리핀어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const review of reviews) {
        // 제목 번역
        if (needsTlPHTranslation(review.title)) {
        ...
        // 내용 번역
        if (needsTlPHTranslation(review.content)) {
        ...
        if (review.concernsMultilingual && needsTlPHTranslation(review.concernsMultilingual)) {
        ...
        for (let i = 0; i < items.length; i += TRANSLATION_BATCH_SIZE) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'title') {
                translationResults[item.id].title_tl_PH = tlPHTranslations[index];
              } else if (item.type === 'content') {
                translationResults[item.id].content_tl_PH = tlPHTranslations[index];
              } else if (item.type === 'concerns') {
                translationResults[item.id].concerns_tl_PH = tlPHTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'title') {
                  translationResults[item.id].title_tl_PH = tlPHTranslation;
                } else if (item.type === 'content') {
                  translationResults[item.id].content_tl_PH = tlPHTranslation;
                } else if (item.type === 'concerns') {
                  translationResults[item.id].concerns_tl_PH = tlPHTranslation;
                }
      ...
      for (const review of reviews) {
        ...
        const translatedReview: TranslatedReview = {
          ...
          title: {
            ...
            hi_IN: titleHiIN,
            tl_PH:
              translations.title_tl_PH ||
              getTextByLocale(review.title, 'tl_PH') ||
              titleKo,
          },
          content: {
            ko_KR: contentKo,
            ...
            tl_PH:
              ...
              getTextByLocale(review.content, 'tl_PH') ||
              contentKo,
          },
        };
        ...
        if (review.concernsMultilingual) {
          ...
          translatedReview.concernsMultilingual = {
            ...
            hi_IN: concernsHiIN,
            tl_PH:
              translations.concerns_tl_PH ||
              getTextByLocale(review.concernsMultilingual, 'tl_PH') ||
              concernsKo,
          };
        }
