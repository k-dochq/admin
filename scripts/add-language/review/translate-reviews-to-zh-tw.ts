...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-reviews-zh-tw.json');
 ...
 * 메인 번역 함수
 */
async function translateReviewsToZhTW() {
  try {
    console.log('🌐 리뷰 중국어 번체(zh-TW) 번역 작업 시작...');
    ...
    // 번역이 필요한 리뷰만 필터링 (zh_TW가 없거나 비어있는 경우)
    const reviewsToTranslate = allReviews.filter((review) => {
      ...
      return (
        needsZhTWTranslation(review.title) ||
        needsZhTWTranslation(review.content) ||
        (review.concernsMultilingual && needsZhTWTranslation(review.concernsMultilingual))
      );
    });

    const totalCount = reviewsToTranslate.length;
    console.log(
      `📊 총 ${allReviews.length}개 리뷰 중 ${totalCount}개 리뷰가 중국어 번체 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const review of reviews) {
        // 제목 번역
        if (needsZhTWTranslation(review.title)) {
        ...
        // 내용 번역
        if (needsZhTWTranslation(review.content)) {
        ...
        if (review.concernsMultilingual && needsZhTWTranslation(review.concernsMultilingual)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'title') {
                ...
                translationResults[item.id].concerns_zh_TW = zhTWTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'title') {
                  ...
                  translationResults[item.id].concerns_zh_TW = zhTWTranslation;
                }
      ...
      for (const review of reviews) {
        ...
        const translatedReview: TranslatedReview = {
          ...
          title: {
            ...
            en_US: titleEn,
            th_TH: titleTh,
            zh_TW: translations.title_zh_TW || getTextByLocale(review.title, 'zh_TW') || titleKo,
          },
          content: {
            ko_KR: contentKo,
            en_US: contentEn,
            th_TH: contentTh,
            zh_TW:
              translations.content_zh_TW || getTextByLocale(review.content, 'zh_TW') || contentKo,
          },
        };
        ...
        if (review.concernsMultilingual) {
          ...
          translatedReview.concernsMultilingual = {
            ...
            th_TH: concernsTh,
            zh_TW:
              translations.concerns_zh_TW ||
              getTextByLocale(review.concernsMultilingual, 'zh_TW') ||
              concernsKo,
          };
        }
