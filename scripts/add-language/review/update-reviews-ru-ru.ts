...
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-reviews-ru-ru.json');
...
async function updateReviewsRuRU() {
  try {
    console.log('🔄 리뷰 러시아어(ru-RU) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedReview) => {
        try {
          ...
          if (translatedReview.title.ru_RU) {
            updateData.title = mergeLocalizedTextWithRuRU(
              currentReview.title,
              translatedReview.title.ru_RU,
            );
          }
          if (translatedReview.content.ru_RU) {
            updateData.content = mergeLocalizedTextWithRuRU(
              currentReview.content,
              translatedReview.content.ru_RU,
            );
          }
          if (
            translatedReview.concernsMultilingual &&
            translatedReview.concernsMultilingual.ru_RU
          ) {
            updateData.concernsMultilingual = mergeLocalizedTextWithRuRU(
