...
const OUTPUT_FILE = path.join(__dirname, 'output/translated-notices-ar-sa.json');
...
async function translateNoticesToArSA() {
  try {
    console.log('🌐 공지사항(Notice) 아랍어(ar-SA) 번역 작업 시작...');
    ...
    const noticesToTranslate = allNotices.filter((notice) => {
      ...
      return needsArSATranslation(notice.title) || needsArSATranslation(notice.content);
    });

    const totalCount = noticesToTranslate.length;
    console.log(
      `📊 총 ${allNotices.length}개 공지사항 중 ${totalCount}개 공지사항이 아랍어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const notice of notices) {
        if (needsArSATranslation(notice.title)) {
        ...
        if (needsArSATranslation(notice.content)) {
      ...
      const translationResults: {
        [key: string]: { title_ar_SA?: string; content_ar_SA?: string };
      } = {};
      ...
      for (const notice of notices) {
        ...
        const translatedNotice: TranslatedNotice = {
          ...
          title: {
            ...
            hi_IN: titleHiIN,
            tl_PH: titleTlPH,
            ar_SA: tr.title_ar_SA || getTextByLocale(notice.title, 'ar_SA') || titleKo,
          },
          content: {
            ko_KR: contentKo,
            en_US: contentEn,
            ...
            hi_IN: contentHiIN,
            tl_PH: contentTlPH,
            ar_SA: tr.content_ar_SA || getTextByLocale(notice.content, 'ar_SA') || contentKo,
          },
        };
