...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-notices-ja-jp.json');
 ...
 * 메인 번역 함수
 */
async function translateNoticesToJaJP() {
  try {
    console.log('🌐 공지사항(Notice) 일본어(ja-JP) 번역 작업 시작...');
    ...
    // 번역이 필요한 공지사항만 필터링 (ja_JP가 없거나 비어있는 경우)
    const noticesToTranslate = allNotices.filter((notice) => {
      ...
      return needsJaJPTranslation(notice.title) || needsJaJPTranslation(notice.content);
    });

    const totalCount = noticesToTranslate.length;
    console.log(
      `📊 총 ${allNotices.length}개 공지사항 중 ${totalCount}개 공지사항이 일본어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const notice of notices) {
        // title 번역
        if (needsJaJPTranslation(notice.title)) {
        ...
        // content 번역
        if (needsJaJPTranslation(notice.content)) {
      ...
      for (const notice of notices) {
        ...
        const translatedNotice: TranslatedNotice = {
          ...
          title: {
            ...
            th_TH: titleTh,
            zh_TW: titleZhTW,
            ja_JP: translations.title_ja_JP || getTextByLocale(notice.title, 'ja_JP') || titleKo,
          },
          content: {
            ko_KR: contentKo,
            en_US: contentEn,
            ...
            zh_TW: contentZhTW,
            ja_JP:
              translations.content_ja_JP || getTextByLocale(notice.content, 'ja_JP') || contentKo,
          },
        };
