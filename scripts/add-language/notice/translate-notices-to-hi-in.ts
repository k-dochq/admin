...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-notices-hi-in.json');
 ...
 * 메인 번역 함수
 */
async function translateNoticesToHiIN() {
  try {
    console.log('🌐 공지사항(Notice) 힌디어(hi-IN) 번역 작업 시작...');
    ...
    // 번역이 필요한 공지사항만 필터링 (hi_IN이 없거나 비어있는 경우)
    const noticesToTranslate = allNotices.filter((notice) => {
      ...
      return needsHiINTranslation(notice.title) || needsHiINTranslation(notice.content);
    });

    const totalCount = noticesToTranslate.length;
    console.log(
      `📊 총 ${allNotices.length}개 공지사항 중 ${totalCount}개 공지사항이 힌디어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const notice of notices) {
        // title 번역
        if (needsHiINTranslation(notice.title)) {
        ...
        // content 번역
        if (needsHiINTranslation(notice.content)) {
      ...
      for (const notice of notices) {
        ...
        const translatedNotice: TranslatedNotice = {
          ...
          title: {
            ...
            zh_TW: titleZhTW,
            ja_JP: titleJaJP,
            hi_IN: translations.title_hi_IN || getTextByLocale(notice.title, 'hi_IN') || titleKo,
          },
          content: {
            ko_KR: contentKo,
            en_US: contentEn,
            ...
            ja_JP: contentJaJP,
            hi_IN:
              translations.content_hi_IN || getTextByLocale(notice.content, 'hi_IN') || contentKo,
          },
        };
