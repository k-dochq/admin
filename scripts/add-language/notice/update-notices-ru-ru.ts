...
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-notices-ru-ru.json');
...
async function updateNoticesRuRU() {
  try {
    console.log('🔄 공지사항(Notice) 러시아어(ru-RU) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedNotice) => {
        try {
          ...
          const updateData: { title?: LocalizedText; content?: LocalizedText } = {};

          if (translatedNotice.title.ru_RU) {
            updateData.title = mergeLocalizedTextWithRuRU(
              currentNotice.title,
              translatedNotice.title.ru_RU,
            );
          }
          if (translatedNotice.content.ru_RU) {
            updateData.content = mergeLocalizedTextWithRuRU(
              currentNotice.content,
              translatedNotice.content.ru_RU,
            );
          }
