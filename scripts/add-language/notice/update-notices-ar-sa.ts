...
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-notices-ar-sa.json');
...
async function updateNoticesArSA() {
  try {
    console.log('🔄 공지사항(Notice) 아랍어(ar-SA) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedNotice) => {
        try {
          ...
          if (translatedNotice.title.ar_SA) {
            updateData.title = mergeLocalizedTextWithArSA(
              currentNotice.title,
              translatedNotice.title.ar_SA,
            );
          }
          if (translatedNotice.content.ar_SA) {
            updateData.content = mergeLocalizedTextWithArSA(
              currentNotice.content,
              translatedNotice.content.ar_SA,
            );
          }
