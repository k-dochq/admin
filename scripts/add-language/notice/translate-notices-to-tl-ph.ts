...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-notices-tl-ph.json');
 ...
 * 메인 번역 함수
 */
async function translateNoticesToTlPH() {
  try {
    console.log('🌐 공지사항(Notice) 필리핀어(tl-PH) 번역 작업 시작...');
    ...
    // 번역이 필요한 공지사항만 필터링 (tl_PH가 없거나 비어있는 경우)
    const noticesToTranslate = allNotices.filter((notice) => {
      ...
      return needsTlPHTranslation(notice.title) || needsTlPHTranslation(notice.content);
    });

    const totalCount = noticesToTranslate.length;
    console.log(
      `📊 총 ${allNotices.length}개 공지사항 중 ${totalCount}개 공지사항이 필리핀어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const notice of notices) {
        // title 번역
        if (needsTlPHTranslation(notice.title)) {
        ...
        // content 번역
        if (needsTlPHTranslation(notice.content)) {
        ...
        const translatedNotice: TranslatedNotice = {
          ...
          title: {
            ...
            tl_PH:
              ...
              getTextByLocale(notice.title, 'tl_PH') ||
              titleKo,
          },
          content: {
            ko_KR: contentKo,
            ...
            tl_PH:
              ...
              getTextByLocale(notice.content, 'tl_PH') ||
              contentKo,
          },
        };
