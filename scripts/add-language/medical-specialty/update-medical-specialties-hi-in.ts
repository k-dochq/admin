...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-medical-specialties-hi-in.json');
 ...
 * 메인 업데이트 함수
 */
async function updateMedicalSpecialtiesHiIN() {
  try {
    console.log('🔄 시술부위(MedicalSpecialty) 힌디어(hi-IN) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedMedicalSpecialty) => {
        try {
          ...
          if (translatedMedicalSpecialty.name.hi_IN) {
            updateData.name = mergeLocalizedTextWithHiIN(
              currentMedicalSpecialty.name,
              translatedMedicalSpecialty.name.hi_IN,
            );
          }
          ...
          if (
            ...
            translatedMedicalSpecialty.description.hi_IN
          ) {
...
export { updateMedicalSpecialtiesHiIN };
