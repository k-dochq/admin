...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-medical-specialties-hi-in.json');
 ...
 * 메인 번역 함수
 */
async function translateMedicalSpecialtiesToHiIN() {
  try {
    console.log('🌐 시술부위(MedicalSpecialty) 힌디어(hi-IN) 번역 작업 시작...');
    ...
    // 번역이 필요한 시술부위만 필터링 (hi_IN이 없거나 비어있는 경우)
    const medicalSpecialtiesToTranslate = allMedicalSpecialties.filter((medicalSpecialty) => {
      ...
      return (
        needsHiINTranslation(medicalSpecialty.name) ||
        (medicalSpecialty.description && needsHiINTranslation(medicalSpecialty.description))
      );
    });

    const totalCount = medicalSpecialtiesToTranslate.length;
    console.log(
      `📊 총 ${allMedicalSpecialties.length}개 시술부위 중 ${totalCount}개 시술부위가 힌디어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const medicalSpecialty of medicalSpecialties) {
        // name 번역
        if (needsHiINTranslation(medicalSpecialty.name)) {
        ...
        // description 번역
        if (medicalSpecialty.description && needsHiINTranslation(medicalSpecialty.description)) {
      ...
      for (const medicalSpecialty of medicalSpecialties) {
        ...
        const translatedMedicalSpecialty: TranslatedMedicalSpecialty = {
          ...
          name: {
            ...
            ja_JP: nameJaJP,
            hi_IN:
              translations.name_hi_IN || getTextByLocale(medicalSpecialty.name, 'hi_IN') || nameKo,
          },
        };
        ...
        if (medicalSpecialty.description) {
          ...
          translatedMedicalSpecialty.description = {
            ...
            hi_IN:
              ...
              getTextByLocale(medicalSpecialty.description, 'hi_IN') ||
              descriptionKo,
          };
        }
...
if (require.main === module) {
  translateMedicalSpecialtiesToHiIN()
    .then(() => {
...
export { translateMedicalSpecialtiesToHiIN };
