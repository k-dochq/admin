...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-medical-specialties-zh-tw.json');
 ...
 * 메인 번역 함수
 */
async function translateMedicalSpecialtiesToZhTW() {
  try {
    console.log('🌐 시술부위(MedicalSpecialty) 중국어 번체(zh-TW) 번역 작업 시작...');
    ...
    // 번역이 필요한 시술부위만 필터링 (zh_TW가 없거나 비어있는 경우)
    const medicalSpecialtiesToTranslate = allMedicalSpecialties.filter((medicalSpecialty) => {
      ...
      return (
        needsZhTWTranslation(medicalSpecialty.name) ||
        (medicalSpecialty.description && needsZhTWTranslation(medicalSpecialty.description))
      );
    });

    const totalCount = medicalSpecialtiesToTranslate.length;
    console.log(
      `📊 총 ${allMedicalSpecialties.length}개 시술부위 중 ${totalCount}개 시술부위가 중국어 번체 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const medicalSpecialty of medicalSpecialties) {
        // name 번역
        if (needsZhTWTranslation(medicalSpecialty.name)) {
        ...
        // description 번역
        if (medicalSpecialty.description && needsZhTWTranslation(medicalSpecialty.description)) {
      ...
      for (const medicalSpecialty of medicalSpecialties) {
        ...
        const translatedMedicalSpecialty: TranslatedMedicalSpecialty = {
          ...
          name: {
            ...
            th_TH: nameTh,
            zh_TW:
              translations.name_zh_TW || getTextByLocale(medicalSpecialty.name, 'zh_TW') || nameKo,
          },
        };
        ...
        if (medicalSpecialty.description) {
          ...
          translatedMedicalSpecialty.description = {
            ...
            zh_TW:
              ...
              getTextByLocale(medicalSpecialty.description, 'zh_TW') ||
              descriptionKo,
          };
        }
...
if (require.main === module) {
  translateMedicalSpecialtiesToZhTW()
    .then(() => {
...
export { translateMedicalSpecialtiesToZhTW };
