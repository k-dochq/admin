...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-medical-specialties-tl-ph.json');
 ...
 * 메인 업데이트 함수
 */
async function updateMedicalSpecialtiesTlPH() {
  try {
    console.log('🔄 시술부위(MedicalSpecialty) 필리핀어(tl-PH) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedMedicalSpecialty) => {
        try {
          ...
          if (translatedMedicalSpecialty.name.tl_PH) {
            updateData.name = mergeLocalizedTextWithTlPH(
              currentMedicalSpecialty.name,
              translatedMedicalSpecialty.name.tl_PH,
            );
          }
          ...
          if (
            ...
            translatedMedicalSpecialty.description.tl_PH
          ) {
...
export { updateMedicalSpecialtiesTlPH };
