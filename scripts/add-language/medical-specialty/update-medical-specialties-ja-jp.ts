...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-medical-specialties-ja-jp.json');
 ...
 * 메인 업데이트 함수
 */
async function updateMedicalSpecialtiesJaJP() {
  try {
    console.log('🔄 시술부위(MedicalSpecialty) 일본어(ja-JP) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedMedicalSpecialty) => {
        try {
          ...
          if (translatedMedicalSpecialty.name.ja_JP) {
            updateData.name = mergeLocalizedTextWithJaJP(
              currentMedicalSpecialty.name,
              translatedMedicalSpecialty.name.ja_JP,
            );
          }
          ...
          if (
            ...
            translatedMedicalSpecialty.description.ja_JP
          ) {
...
export { updateMedicalSpecialtiesJaJP };
