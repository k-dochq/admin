...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-medical-specialties-zh-tw.json');
 ...
 * 메인 업데이트 함수
 */
async function updateMedicalSpecialtiesZhTW() {
  try {
    console.log('🔄 시술부위(MedicalSpecialty) 중국어 번체(zh-TW) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedMedicalSpecialty) => {
        try {
          ...
          if (translatedMedicalSpecialty.name.zh_TW) {
            updateData.name = mergeLocalizedTextWithZhTW(
              currentMedicalSpecialty.name,
              translatedMedicalSpecialty.name.zh_TW,
            );
          }
          ...
          if (
            ...
            translatedMedicalSpecialty.description.zh_TW
          ) {
...
export { updateMedicalSpecialtiesZhTW };
