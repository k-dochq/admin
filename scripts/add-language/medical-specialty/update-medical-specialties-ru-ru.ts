...
const TRANSLATION_FILE = path.join(
  __dirname,
  'output/translated-medical-specialties-ru-ru.json',
);
...
async function updateMedicalSpecialtiesRuRU() {
  try {
    console.log('🔄 시술부위(MedicalSpecialty) 러시아어(ru-RU) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedMedicalSpecialty) => {
        try {
          ...
          if (translatedMedicalSpecialty.name.ru_RU) {
            updateData.name = mergeLocalizedTextWithRuRU(
              currentMedicalSpecialty.name,
              translatedMedicalSpecialty.name.ru_RU,
            );
          }
          if (
            translatedMedicalSpecialty.description &&
            translatedMedicalSpecialty.description.ru_RU
          ) {
...
export { updateMedicalSpecialtiesRuRU };
