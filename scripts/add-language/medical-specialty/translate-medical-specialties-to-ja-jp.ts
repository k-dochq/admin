...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-medical-specialties-ja-jp.json');
 ...
 * 메인 번역 함수
 */
async function translateMedicalSpecialtiesToJaJP() {
  try {
    console.log('🌐 시술부위(MedicalSpecialty) 일본어(ja-JP) 번역 작업 시작...');
    ...
    // 번역이 필요한 시술부위만 필터링 (ja_JP가 없거나 비어있는 경우)
    const medicalSpecialtiesToTranslate = allMedicalSpecialties.filter((medicalSpecialty) => {
      ...
      return (
        needsJaJPTranslation(medicalSpecialty.name) ||
        (medicalSpecialty.description && needsJaJPTranslation(medicalSpecialty.description))
      );
    });

    const totalCount = medicalSpecialtiesToTranslate.length;
    console.log(
      `📊 총 ${allMedicalSpecialties.length}개 시술부위 중 ${totalCount}개 시술부위가 일본어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const medicalSpecialty of medicalSpecialties) {
        // name 번역
        if (needsJaJPTranslation(medicalSpecialty.name)) {
        ...
        // description 번역
        if (medicalSpecialty.description && needsJaJPTranslation(medicalSpecialty.description)) {
      ...
      for (const medicalSpecialty of medicalSpecialties) {
        ...
        const translatedMedicalSpecialty: TranslatedMedicalSpecialty = {
          ...
          name: {
            ...
            zh_TW: nameZhTW,
            ja_JP:
              translations.name_ja_JP || getTextByLocale(medicalSpecialty.name, 'ja_JP') || nameKo,
          },
        };
        ...
        if (medicalSpecialty.description) {
          ...
          translatedMedicalSpecialty.description = {
            ...
            ja_JP:
              ...
              getTextByLocale(medicalSpecialty.description, 'ja_JP') ||
              descriptionKo,
          };
        }
...
if (require.main === module) {
  translateMedicalSpecialtiesToJaJP()
    .then(() => {
...
export { translateMedicalSpecialtiesToJaJP };
