...
const OUTPUT_FILE = path.join(__dirname, 'output/translated-medical-specialties-ar-sa.json');
...
async function translateMedicalSpecialtiesToArSA() {
  try {
    console.log('🌐 시술부위(MedicalSpecialty) 아랍어(ar-SA) 번역 작업 시작...');
    ...
    const medicalSpecialtiesToTranslate = allMedicalSpecialties.filter((ms) => {
      ...
      return (
        needsArSATranslation(ms.name) || (ms.description && needsArSATranslation(ms.description))
      );
    });

    const totalCount = medicalSpecialtiesToTranslate.length;
    console.log(
      `📊 총 ${allMedicalSpecialties.length}개 시술부위 중 ${totalCount}개 시술부위가 아랍어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const ms of medicalSpecialties) {
        if (needsArSATranslation(ms.name)) {
        ...
        if (ms.description && needsArSATranslation(ms.description)) {
      ...
      const translationResults: {
        [key: string]: { name_ar_SA?: string; description_ar_SA?: string };
      } = {};
      ...
      for (const ms of medicalSpecialties) {
        ...
        const translatedMedicalSpecialty: TranslatedMedicalSpecialty = {
          ...
          name: {
            ...
            hi_IN: nameHiIN,
            tl_PH: nameTlPH,
            ar_SA: tr.name_ar_SA || getTextByLocale(ms.name, 'ar_SA') || nameKo,
          },
        };
        ...
        if (ms.description) {
          translatedMedicalSpecialty.description = {
            ...
            ar_SA:
              ...
              getTextByLocale(ms.description, 'ar_SA') ||
...
if (require.main === module) {
  translateMedicalSpecialtiesToArSA()
    .then(() => {
...
export { translateMedicalSpecialtiesToArSA };
