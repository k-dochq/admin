...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-doctors-zh-tw.json');
 ...
 * 번역 결과 파일을 로드하는 함수
 */
function loadTranslationResults(): TranslatedDoctor[] {
  try {
 ...
 * 메인 업데이트 함수
 */
async function updateDoctorsZhTW() {
  try {
    console.log('🔄 의사 중국어 번체(zh-TW) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      // 배치별 업데이트 처리
      const updatePromises = batch.map(async (translatedDoctor) => {
        try {
          // 현재 의사 데이터 조회
          const currentDoctor = await prisma.doctor.findUnique({
            where: { id: translatedDoctor.id },
            select: {
              id: true,
              name: true,
              position: true,
          ...
          if (!currentDoctor) {
            console.warn(`⚠️  의사를 찾을 수 없습니다: ${translatedDoctor.id}`);
            return { success: false, id: translatedDoctor.id, error: 'Doctor not found' };
          }
          ...
          // name 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedDoctor.name.zh_TW) {
            updateData.name = mergeLocalizedTextWithZhTW(
              currentDoctor.name,
              translatedDoctor.name.zh_TW,
            );
          }

          // position 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedDoctor.position && translatedDoctor.position.zh_TW) {
            updateData.position = mergeLocalizedTextWithZhTW(
              currentDoctor.position,
              translatedDoctor.position.zh_TW,
            );
          }

          // career 업데이트 (기존 데이터 유지하고 zh_TW만 추가)
          if (translatedDoctor.career && translatedDoctor.career.zh_TW) {
            updateData.career = mergeLocalizedTextWithZhTW(
              currentDoctor.career,
              translatedDoctor.career.zh_TW,
            );
          }
          ...
          return { success: true, id: translatedDoctor.id };
        } catch (error) {
          console.error(`❌ 업데이트 실패 (${translatedDoctor.id}):`, error);
          return {
            success: false,
            id: translatedDoctor.id,
