...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-doctors-zh-tw.json');
 ...
 * 메인 번역 함수
 */
async function translateDoctorsToZhTW() {
  try {
    console.log('🌐 의사 중국어 번체(zh-TW) 번역 작업 시작...');
    ...
    // 번역이 필요한 의사만 필터링 (zh_TW가 없거나 비어있는 경우)
    const doctorsToTranslate = allDoctors.filter((doctor) => {
      ...
      return (
        needsZhTWTranslation(doctor.name) ||
        (doctor.position && needsZhTWTranslation(doctor.position)) ||
        (doctor.career && needsZhTWTranslation(doctor.career))
      );
    });

    const totalCount = doctorsToTranslate.length;
    console.log(
      `📊 총 ${allDoctors.length}개 의사 중 ${totalCount}개 의사가 중국어 번체 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const doctor of doctors) {
        // name 번역
        if (needsZhTWTranslation(doctor.name)) {
        ...
        // position 번역
        if (doctor.position && needsZhTWTranslation(doctor.position)) {
        ...
        // career 번역
        if (doctor.career && needsZhTWTranslation(doctor.career)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].position_zh_TW = zhTWTranslations[index];
              } else if (item.type === 'career') {
                translationResults[item.id].career_zh_TW = zhTWTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].position_zh_TW = zhTWTranslation;
                } else if (item.type === 'career') {
                  translationResults[item.id].career_zh_TW = zhTWTranslation;
                }
      ...
      for (const doctor of doctors) {
        ...
        const translatedDoctor: TranslatedDoctor = {
          ...
          name: {
            ...
            en_US: nameEn,
            th_TH: nameTh,
            zh_TW: translations.name_zh_TW || getTextByLocale(doctor.name, 'zh_TW') || nameKo,
          },
        };
        ...
        if (doctor.position) {
          ...
          translatedDoctor.position = {
            ...
            th_TH: positionTh,
            zh_TW:
              translations.position_zh_TW ||
              getTextByLocale(doctor.position, 'zh_TW') ||
              positionKo,
          };
        }
        ...
        if (doctor.career) {
          ...
          translatedDoctor.career = {
            ...
            en_US: careerEn,
            th_TH: careerTh,
            zh_TW: translations.career_zh_TW || getTextByLocale(doctor.career, 'zh_TW') || careerKo,
          };
        }
