...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-doctors-tl-ph.json');
 ...
 * 메인 번역 함수
 */
async function translateDoctorsToTlPH() {
  try {
    console.log('🌐 의사 필리핀어(tl-PH) 번역 작업 시작...');
    ...
    // 번역이 필요한 의사만 필터링 (tl_PH가 없거나 비어있는 경우)
    const doctorsToTranslate = allDoctors.filter((doctor) => {
      ...
      return (
        needsTlPHTranslation(doctor.name) ||
        (doctor.position && needsTlPHTranslation(doctor.position)) ||
        (doctor.career && needsTlPHTranslation(doctor.career))
      );
    });

    const totalCount = doctorsToTranslate.length;
    console.log(
      `📊 총 ${allDoctors.length}개 의사 중 ${totalCount}개 의사가 필리핀어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const doctor of doctors) {
        // name 번역
        if (needsTlPHTranslation(doctor.name)) {
        ...
        // position 번역
        if (doctor.position && needsTlPHTranslation(doctor.position)) {
        ...
        // career 번역
        if (doctor.career && needsTlPHTranslation(doctor.career)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].position_tl_PH = tlPHTranslations[index];
              } else if (item.type === 'career') {
                translationResults[item.id].career_tl_PH = tlPHTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].position_tl_PH = tlPHTranslation;
                } else if (item.type === 'career') {
                  translationResults[item.id].career_tl_PH = tlPHTranslation;
                }
      ...
      for (const doctor of doctors) {
        ...
        const translatedDoctor: TranslatedDoctor = {
          ...
          name: {
            ...
            ja_JP: nameJaJP,
            hi_IN: nameHiIN,
            tl_PH: translations.name_tl_PH || getTextByLocale(doctor.name, 'tl_PH') || nameKo,
          },
        };
        ...
        if (doctor.position) {
          ...
          translatedDoctor.position = {
            ...
            hi_IN: positionHiIN,
            tl_PH:
              translations.position_tl_PH ||
              getTextByLocale(doctor.position, 'tl_PH') ||
              positionKo,
          };
        }
        ...
        if (doctor.career) {
          ...
          translatedDoctor.career = {
            ...
            hi_IN: careerHiIN,
            tl_PH:
              translations.career_tl_PH || getTextByLocale(doctor.career, 'tl_PH') || careerKo,
          };
        }
