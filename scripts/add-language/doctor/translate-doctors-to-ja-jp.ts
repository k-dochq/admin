...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-doctors-ja-jp.json');
 ...
 * 메인 번역 함수
 */
async function translateDoctorsToJaJP() {
  try {
    console.log('🌐 의사 일본어(ja-JP) 번역 작업 시작...');
    ...
    // 번역이 필요한 의사만 필터링 (ja_JP가 없거나 비어있는 경우)
    const doctorsToTranslate = allDoctors.filter((doctor) => {
      ...
      return (
        needsJaJPTranslation(doctor.name) ||
        (doctor.position && needsJaJPTranslation(doctor.position)) ||
        (doctor.career && needsJaJPTranslation(doctor.career))
      );
    });

    const totalCount = doctorsToTranslate.length;
    console.log(
      `📊 총 ${allDoctors.length}개 의사 중 ${totalCount}개 의사가 일본어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const doctor of doctors) {
        // name 번역
        if (needsJaJPTranslation(doctor.name)) {
        ...
        // position 번역
        if (doctor.position && needsJaJPTranslation(doctor.position)) {
        ...
        // career 번역
        if (doctor.career && needsJaJPTranslation(doctor.career)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].position_ja_JP = jaJPTranslations[index];
              } else if (item.type === 'career') {
                translationResults[item.id].career_ja_JP = jaJPTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].position_ja_JP = jaJPTranslation;
                } else if (item.type === 'career') {
                  translationResults[item.id].career_ja_JP = jaJPTranslation;
                }
      ...
      for (const doctor of doctors) {
        ...
        const translatedDoctor: TranslatedDoctor = {
          ...
          name: {
            ...
            th_TH: nameTh,
            zh_TW: nameZhTW,
            ja_JP: translations.name_ja_JP || getTextByLocale(doctor.name, 'ja_JP') || nameKo,
          },
        };
        ...
        if (doctor.position) {
          ...
          translatedDoctor.position = {
            ...
            zh_TW: positionZhTW,
            ja_JP:
              translations.position_ja_JP ||
              getTextByLocale(doctor.position, 'ja_JP') ||
              positionKo,
          };
        }
        ...
        if (doctor.career) {
          ...
          translatedDoctor.career = {
            ...
            th_TH: careerTh,
            zh_TW: careerZhTW,
            ja_JP: translations.career_ja_JP || getTextByLocale(doctor.career, 'ja_JP') || careerKo,
          };
        }
