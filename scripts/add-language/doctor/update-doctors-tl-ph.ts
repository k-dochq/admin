...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-doctors-tl-ph.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDoctorsTlPH() {
  try {
    console.log('🔄 의사 필리핀어(tl-PH) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDoctor) => {
        try {
          ...
          // name 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedDoctor.name.tl_PH) {
            updateData.name = mergeLocalizedTextWithTlPH(
              currentDoctor.name,
              translatedDoctor.name.tl_PH,
            );
          }

          // position 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedDoctor.position && translatedDoctor.position.tl_PH) {
            updateData.position = mergeLocalizedTextWithTlPH(
              currentDoctor.position,
              translatedDoctor.position.tl_PH,
            );
          }

          // career 업데이트 (기존 데이터 유지하고 tl_PH만 추가)
          if (translatedDoctor.career && translatedDoctor.career.tl_PH) {
            updateData.career = mergeLocalizedTextWithTlPH(
              currentDoctor.career,
              translatedDoctor.career.tl_PH,
            );
          }
