...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-doctors-hi-in.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDoctorsHiIN() {
  try {
    console.log('🔄 의사 힌디어(hi-IN) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDoctor) => {
        try {
          ...
          // name 업데이트 (기존 데이터 유지하고 hi_IN만 추가)
          if (translatedDoctor.name.hi_IN) {
            updateData.name = mergeLocalizedTextWithHiIN(
              currentDoctor.name,
              translatedDoctor.name.hi_IN,
            );
          }

          // position 업데이트 (기존 데이터 유지하고 hi_IN만 추가)
          if (translatedDoctor.position && translatedDoctor.position.hi_IN) {
            updateData.position = mergeLocalizedTextWithHiIN(
              currentDoctor.position,
              translatedDoctor.position.hi_IN,
            );
          }

          // career 업데이트 (기존 데이터 유지하고 hi_IN만 추가)
          if (translatedDoctor.career && translatedDoctor.career.hi_IN) {
            updateData.career = mergeLocalizedTextWithHiIN(
              currentDoctor.career,
              translatedDoctor.career.hi_IN,
            );
          }
