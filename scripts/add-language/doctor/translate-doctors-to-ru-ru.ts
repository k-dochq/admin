...
// 결과 파일 경로
const OUTPUT_FILE = path.join(__dirname, 'output/translated-doctors-ru-ru.json');
 ...
 * 메인 번역 함수
 */
async function translateDoctorsToRuRU() {
  try {
    console.log('🌐 의사 러시아어(ru-RU) 번역 작업 시작...');
    ...
    // 번역이 필요한 의사만 필터링 (ru_RU가 없거나 비어있는 경우)
    const doctorsToTranslate = allDoctors.filter((doctor) => {
      ...
      return (
        needsRuRUTranslation(doctor.name) ||
        (doctor.position && needsRuRUTranslation(doctor.position)) ||
        (doctor.career && needsRuRUTranslation(doctor.career))
      );
    });

    const totalCount = doctorsToTranslate.length;
    console.log(
      `📊 총 ${allDoctors.length}개 의사 중 ${totalCount}개 의사가 러시아어 번역이 필요합니다.`,
    );
    ...
    while (processedCount < totalCount) {
      ...
      for (const doctor of doctors) {
        // name 번역
        if (needsRuRUTranslation(doctor.name)) {
        ...
        // position 번역
        if (doctor.position && needsRuRUTranslation(doctor.position)) {
        ...
        // career 번역
        if (doctor.career && needsRuRUTranslation(doctor.career)) {
          ...
          try {
            ...
            batch.forEach((item, index) => {
              ...
              if (item.type === 'name') {
                ...
                translationResults[item.id].position_ru_RU = ruRUTranslations[index];
              } else if (item.type === 'career') {
                translationResults[item.id].career_ru_RU = ruRUTranslations[index];
              }
            });
            ...
            for (const item of batch) {
              try {
                ...
                if (item.type === 'name') {
                  ...
                  translationResults[item.id].position_ru_RU = ruRUTranslation;
                } else if (item.type === 'career') {
                  translationResults[item.id].career_ru_RU = ruRUTranslation;
                }
      ...
      for (const doctor of doctors) {
        ...
        const nameArSA = getTextByLocale(doctor.name, 'ar_SA');
        ...
        const translatedDoctor: TranslatedDoctor = {
          ...
          name: {
            ...
            tl_PH: nameTlPH,
            ar_SA: nameArSA,
            ru_RU: translations.name_ru_RU || getTextByLocale(doctor.name, 'ru_RU') || nameKo,
          },
        };
        ...
        if (doctor.position) {
          ...
          const positionArSA = getTextByLocale(doctor.position, 'ar_SA');
          ...
          translatedDoctor.position = {
            ...
            ar_SA: positionArSA,
            ru_RU:
              translations.position_ru_RU ||
              getTextByLocale(doctor.position, 'ru_RU') ||
              positionKo,
          };
        }
        ...
        if (doctor.career) {
          ...
          const careerArSA = getTextByLocale(doctor.career, 'ar_SA');
          ...
          translatedDoctor.career = {
            ...
            ar_SA: careerArSA,
            ru_RU:
              translations.career_ru_RU || getTextByLocale(doctor.career, 'ru_RU') || careerKo,
          };
        }
