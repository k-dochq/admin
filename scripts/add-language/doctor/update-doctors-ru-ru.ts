...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-doctors-ru-ru.json');
...
/**
 * 기존 LocalizedText에 ru_RU만 추가/업데이트하는 함수
 */
 ...
 * 메인 업데이트 함수
 */
async function updateDoctorsRuRU() {
  try {
    console.log('🔄 의사 러시아어(ru-RU) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDoctor) => {
        try {
          ...
          // name 업데이트 (기존 데이터 유지하고 ru_RU만 추가)
          if (translatedDoctor.name.ru_RU) {
            updateData.name = mergeLocalizedTextWithRuRU(
              currentDoctor.name,
              translatedDoctor.name.ru_RU,
            );
          }

          // position 업데이트 (기존 데이터 유지하고 ru_RU만 추가)
          if (translatedDoctor.position && translatedDoctor.position.ru_RU) {
            updateData.position = mergeLocalizedTextWithRuRU(
              currentDoctor.position,
              translatedDoctor.position.ru_RU,
            );
          }

          // career 업데이트 (기존 데이터 유지하고 ru_RU만 추가)
          if (translatedDoctor.career && translatedDoctor.career.ru_RU) {
            updateData.career = mergeLocalizedTextWithRuRU(
              currentDoctor.career,
              translatedDoctor.career.ru_RU,
            );
          }
