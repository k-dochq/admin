...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-doctors-ja-jp.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDoctorsJaJP() {
  try {
    console.log('🔄 의사 일본어(ja-JP) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDoctor) => {
        try {
          ...
          // name 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (translatedDoctor.name.ja_JP) {
            updateData.name = mergeLocalizedTextWithJaJP(
              currentDoctor.name,
              translatedDoctor.name.ja_JP,
            );
          }

          // position 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (translatedDoctor.position && translatedDoctor.position.ja_JP) {
            updateData.position = mergeLocalizedTextWithJaJP(
              currentDoctor.position,
              translatedDoctor.position.ja_JP,
            );
          }

          // career 업데이트 (기존 데이터 유지하고 ja_JP만 추가)
          if (translatedDoctor.career && translatedDoctor.career.ja_JP) {
            updateData.career = mergeLocalizedTextWithJaJP(
              currentDoctor.career,
              translatedDoctor.career.ja_JP,
            );
          }
