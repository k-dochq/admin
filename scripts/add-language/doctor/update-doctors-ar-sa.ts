...
// 번역 결과 파일 경로
const TRANSLATION_FILE = path.join(__dirname, 'output/translated-doctors-ar-sa.json');
 ...
 * 메인 업데이트 함수
 */
async function updateDoctorsArSA() {
  try {
    console.log('🔄 의사 아랍어(ar-SA) 업데이트 작업 시작...');
    ...
    while (startIndex < translationResults.length) {
      ...
      const updatePromises = batch.map(async (translatedDoctor) => {
        try {
          ...
          if (translatedDoctor.name.ar_SA) {
            updateData.name = mergeLocalizedTextWithArSA(
              currentDoctor.name,
              translatedDoctor.name.ar_SA,
            );
          }

          // position 업데이트 (기존 데이터 유지하고 ar_SA만 추가)
          if (translatedDoctor.position && translatedDoctor.position.ar_SA) {
            updateData.position = mergeLocalizedTextWithArSA(
              currentDoctor.position,
              translatedDoctor.position.ar_SA,
            );
          }

          // career 업데이트 (기존 데이터 유지하고 ar_SA만 추가)
          if (translatedDoctor.career && translatedDoctor.career.ar_SA) {
            updateData.career = mergeLocalizedTextWithArSA(
              currentDoctor.career,
              translatedDoctor.career.ar_SA,
            );
          }
