/**
 * 병원 표시지역명(displayLocationName) 중 tl_PH(Filipino)만 매핑 정보로 업데이트
 *
 * - 기준: displayLocationName.ko_KR(한국어 지역명)
 * - 매핑: scripts/district-name/location-mapping.ts 의 LOCATION_NAME_MAPPING
 * - 동작: tl_PH 값이 매핑과 다르면 tl_PH만 교체(다른 언어는 유지)
 *
 * 실행 예시:
 *  - DRY RUN:   pnpm -C . db:update-hospital-location-names-tl-ph -- --dry-run
 *  - 제한 실행: pnpm -C . db:update-hospital-location-names-tl-ph -- --limit 50 --batch 20
 *
 * 주의:
...
const TARGET_LOCALE: Locale = 'tl_PH';
...
async function updateHospitalTlPh({
  hospitalId,
  displayLocationName,
  expectedTlPh,
  dryRun,
  ...
  const currentTlPh = getLocalizedText(displayLocationName, TARGET_LOCALE);
  if (currentTlPh === expectedTlPh) return { updated: false };

  if (dryRun) {
    console.log(
      `  [DRY RUN] ${TARGET_LOCALE}: "${currentTlPh || '(없음)'}" → "${expectedTlPh}"`,
    );
    return { updated: true };
  }

  const updatedDisplayLocationName = mergeLocalizedText(displayLocationName, TARGET_LOCALE, expectedTlPh);
...
async function main(): Promise<void> {
  ...
  console.log('🔄 tl_PH 표시지역명 매핑 업데이트 시작');
  ...
  while (true) {
    ...
    for (const hospital of hospitals) {
      ...
      const expectedTlPh = mapping?.[TARGET_LOCALE] ?? '';
      if (!expectedTlPh) {
        skipped += 1;
        continue;
      }

      const result = await updateHospitalTlPh({
...
export { main as updateHospitalLocationNamesTlPh };
