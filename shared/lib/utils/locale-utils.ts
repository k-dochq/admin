 ...
 * @returns 언어 코드 ('ko' | 'en' | 'th' | 'zh' | 'ja' | 'hi' | 'tl' | 'ar')
 */
export function localeToLangCode(locale: HospitalLocale): 'ko' | 'en' | 'th' | 'zh' | 'ja' | 'hi' | 'tl' | 'ar' {
 ...
 * 우선순위: ko_KR > en_US > th_TH > zh_TW > ja_JP > hi_IN > tl_PH > ar_SA
 * @param text - LocalizedText 객체
...
export function getFirstAvailableText(text: LocalizedText | null | undefined): string {
  ...
  return text.ko_KR || text.en_US || text.th_TH || text.zh_TW || text.ja_JP || text.hi_IN || text.tl_PH || text.ar_SA || '';
}
